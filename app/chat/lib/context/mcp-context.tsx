...
export function MCPProvider(props: {
  ...
  const { serverNameText, serverUrl, serverName } = useMemo(() => {
    ...
    return {
      ...
      serverUrl: ["https://mcpstorefront.com", owner, repo]
        .filter(Boolean)
        .join("/"),
  ...
  const fixedMcpServer = useMemo<MCPServer>(() => {
    return {
      id: ["shopMcp", owner, repo].filter(Boolean).join("-"),
      name: serverName,
      url: serverUrl,
      type: "sse",
      isFixed: true,
  ...
  const {
      ...
      return {
        mcpServers: [
          ...
          ...mcpServersFromLocalStorage.filter(
            (server) => server.id !== fixedMcpServer.id,
          ),
        ],
    ...
    return {
      ...
      selectedMcpServers: [
        fixedMcpServer.id,
        ...selectedMcpServersFromState.filter((id) => id !== fixedMcpServer.id),
      ],
