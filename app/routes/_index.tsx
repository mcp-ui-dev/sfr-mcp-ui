...
import { getActionMode, toStoreDomain } from "../../src/utils/ui-urls";

export const loader = async ({ request }: { request: Request }) => {
  const url = new URL(request.url);
  let actionsMode: "default" | "prompt" | "tool" = "default";
  let proxyMode: boolean = false;
  let storeName = toStoreDomain(url.searchParams.get("store"));
  if (url.searchParams.get("store_domain")) {
    storeName = toStoreDomain(url.searchParams.get("store_domain"));
    actionsMode = "prompt";
    proxyMode = true;
  }
  if (url.searchParams.get("storedomain")) {
    storeName = toStoreDomain(url.searchParams.get("storedomain"));
    actionsMode = "tool";
    proxyMode = true;
  }
